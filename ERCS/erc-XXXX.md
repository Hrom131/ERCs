...
The following data is required to verify the inclusion of a specific transaction within a block:
1. `blockHash`: The hash of the Bitcoin block in which the transaction is to be verified.
2. `txId`: The transaction ID, which is the double `SHA256` hash of the raw transaction.
3. `merkleProof`: An array of sibling hashes that form the path from the transaction hash to the block's Merkle Root.
4. `directions`: An array of hashing directions, where:
    * `0` - The computed hash is on the left, the sibling hash is on the right.
    * `1` - The computed hash is on the right, the sibling hash is on the left.
    * `2` - The node has no sibling and is hashed with itself.

The `merkleProof` and `directions` data are typically generated by an off-chain client or library that queries a Bitcoin full node.

Given all the required data, the Merkle Proof verification process proceeds as follows:
* The `txId` serves as the initial value for hashing.
* The algorithm iterates through the `merkleProof` and `directions` arrays, performing a double `SHA256` hash on the concatenated hashes at each step to move up the Merkle tree.
* The `directions` array dictates the order of concatenation for each hashing operation.

After the final hashing operation, the computed Merkle root MUST be compared with the `Merkle Root` field stored within the block header identified by `blockHash`. If the roots match, the transaction's inclusion in the block is cryptographically verified.
